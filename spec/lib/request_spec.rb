...
RSpec.describe ParliamentLdaWrapper::Request do
  ...
  context '#get_data' do
    context 'research briefings' do
      ...
      it 'gets all the data' do
        VCR.use_cassette('request/get_data/get_all_research_briefings_data') do
          expect(research_briefings_request['result']['items'].count).to eq(1266)
        end
      end

      context 'different options' do
        let(:options){ {'min-date': '2017-01-01', 'max-date': '2017-06-31', 'exists-category': 'true' } }

        it 'will filter using different options' do
          VCR.use_cassette('request/get_data/get_data_different_options') do
            expect(research_briefings_request['result']['items'].count).to eq(96)
          end
        end
      end
    end
    ...
    context 'constituencies' do
      ...
      let(:options)              { {} }
      let(:constituencies_request){ described_class.new(endpoint_string).get_data(options) }

      it 'gets all the data' do
        VCR.use_cassette('request/get_data/get_all_constituencies_data') do
          expect(constituencies_request['result']['items']).to_not eq(nil)
        end
      end

      it 'gets valid data' do
        VCR.use_cassette('request/get_data/get_single_constituency') do
  ...
  context '#get_data_by_ids' do
    let(:ids_request){ described_class.new.get_data_by_ids(ids) }

    context 'multiple valid ids' do
      let(:ids){ ['143468', '143491', '824308'] }

      it 'will return valid data' do
        VCR.use_cassette('request/get_data_by_ids/multiple_valid_ids') do
    ...
    context 'single valid id' do
      let(:ids){ ['143468'] }
      it 'will return valid data' do
        VCR.use_cassette('request/get_data_by_ids/single_valid_id') do
    ...
    context 'invalid ids' do
      context 'no ID' do
        ...
        it 'will raise an error' do
          VCR.use_cassette('request/get_data_by_ids/invalid_ids/empty_array') do
      ...
      context 'ID does not exist' do
        ...
        it 'is a pending example: will not return a result'
          #VCR.use_cassette('request/get_data_by_ids/invalid_ids/id_does_not_exist') 
          #end
        #end
      end
    end
